...
HIDDEN = (
    "python -c \"import config, formatter; "
    "assert config.load({'mode':'fast','timeout':7})['mode']=='fast'; "
    "assert config.load({'timeout':7})['timeout']==7; "
    "assert config.load({'retries':9})['retries']==9; "
    "assert formatter.format_bytes(2048)=='2.0 KB'; "
...
async def run_check(ctx: CheckerContext) -> CheckResult:
    ...
    checks.append(await assert_tests_unmodified(ctx, ["tests/test_config.py"]))
    checks.append(await assert_implementation_modified(ctx, "config.py"))
    checks.append(await assert_implementation_modified(ctx, "formatter.py"))
