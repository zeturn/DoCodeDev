...
The response includes the configured default plus APICred runtime catalog entries when APICred exposes them. Job creation validates requested provider/model pairs against the same policy and still permits the local `scripted` runtime for smoke tests.
