...
def source_progress_forced(state: Any) -> bool:
    if not is_crawler_instruction(state.job.instruction) or successful_source_inspection(state.messages, state.job.instruction) is None:
        return False
    ...
    return max(0, len(identities) - 1) >= MAX_OPTIONAL_PRE_EDIT_SOURCE_URLS or decisions >= MAX_PRE_EDIT_MODEL_DECISIONS


def continuation_allowed(state: Any, phase: Any | None = None) -> bool:
    if not is_crawler_instruction(state.job.instruction) or successful_source_inspection(state.messages, state.job.instruction) is None:
        return False
    if source_progress_forced(state) or unique_network_source_count(state) >= MAX_UNIQUE_SOURCE_INSPECTIONS:
        return False
...
def source_tool_block(state: Any, tool_name: str, args: dict[str, object]) -> str:
    if not is_crawler_instruction(state.job.instruction) or tool_name != "inspect_source":
        return ""
    ...
    if identity and identity in successful_source_identities(state):
        return "duplicate_source_inspection: This source is already available in Source Inspection memory. Do not inspect it again or switch modes to refetch it. Read or edit the implementation now."
    if source_origin(raw_url) in allowed_source_origins(state):
        return ""
    ...
    return f"inspect_source is restricted to the same source origin(s): {origins or 'the task source origin'}."
